...
router.get("/", (req, res, next) => {
  ...
  pool
    .query(query, values)
    .then((result) => {
      res.status(200).send({ success: true, chatids: result.rows });
    })
    .catch((err) => {
      res.status(400).send({
        message: "SQL Error",
...
/**
 * @api {post} /chats Request add a user to a chat room
 * @apiName AddUser
 * @apiGroup Chats
 *
 * @apiHeader {String} authorization valid json web token (JWT)
...
/**
 * @api {get} /chats Request to get info of all users in the chat room
 * @apiName GetUsers
 * @apiGroup Chats
 *
 * @apiHeader {String} authorization valid json web token (JWT)
...
/**
 * @api {delete} /chats Request to delete a user from the chat room
 * @apiName DeleteUser
 * @apiGroup Chats
 *
 * @apiHeader {String} authorization valid json web token (JWT)
