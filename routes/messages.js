...
// put given message into the desired chat
router.post(
    "/",
    ...
    (req, res) => {
        const query =
            "select token from push_token inner join chatmembers on push_token.memberid=chatmembers.memberid where chatmembers.chatid = $1";
        const values = [req.body.chatid];
        ...
        pool.query(query, values)
            .then((result) => {
                ...
                result.rows.forEach((entry) =>
                    msg_functions.sendMessageToIndividual(entry.token, res.message)
                );
                res.status(200).send({
                    success: true,
                });
...
// get chat all  messages from the given chat, or get a specific message in the chat
router.get(
    "/:chatid?/:messageid?",
    ...
    (req, res) => {
        ...
        const values = [req.params.chatid, req.params.messageid];
